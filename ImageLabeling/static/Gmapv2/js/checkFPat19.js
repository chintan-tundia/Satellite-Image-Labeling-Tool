...
/*-------Functions--------*/  
  ...
  function getFPat19FromId(marker_id){    
    $.ajax({
            type:"POST",
                url: 'ajax/getmarkerfromid',
                data: {
                    'marker_id':marker_id  
                },
                dataType: 'json',
                success: function (data) {                
                    if (data.status==1) {                        
                      geomJson=JSON.parse(data.geometryJSON)
                      var worldCoords=geomJson.worldCoords
                      classNm=geomJson["classnm"]
                      var latLngArray=[];
                      ...
                      allObjects[0]=objPolygon
                      objPolygon.setMap(map);
                      centerLatLng=getPolygonCenter(latLngArray);                      
                      var clat=centerLatLng.lat()
                      var clng=centerLatLng.lng()                      
                      //Bounds to shift the center between 1 to 270pixels                      
                      var resolution = 0.298 //for zoom level 19
                      var length_of_tile = 200
                      var length_of_tile = Math.floor((Math.random() * length_of_tile) + 1);                      
                      var latOffset = ((resolution * length_of_tile) * 1) / 111111
                      //var latOffset = 0.0017164817164817
                      //print("LatOffset"+str(latOffset))
                      var lngOffset = latOffset/Math.cos(Math.PI*clat/180)
                      var latFlag=Math.floor((Math.random() * 100) + 1);
                      var lngFlag=Math.floor((Math.random() * 100) + 1);
...
/*-------Event Listeners--------*/
          ...
          /*geocoder.geocode({'latLng': latlng}, function(results, status) {
           if (status == google.maps.GeocoderStatus.OK) {*/        
             /*if (results[0]) {
                ...
                var district=null;
                var idtxt=$('#SelSample option:selected').val()
                var pos=idtxt.indexOf(',')
                var gmapMarkerId=idtxt.substring(1, pos); 
                pos=idtxt.lastIndexOf(',')
                var strend=idtxt.length
                var derivedFromImageId=idtxt.substring(pos+2, strend-1 );                 
                pos=idtxt.indexOf("'")
                var pos1=idtxt.lastIndexOf("'")
                var locality=idtxt.substring(pos+1,pos1)
                console.log(locality)

                $.ajax({
                  type:"POST",
                      url: 'ajax/save_image_fps',
                      data: {
                      ...
                      success: function (data) {                
                          if (data.status==1) {
                            ...
                            //$('#SelDistrictLoc :nth-child(1)').prop('selected', true);
                            //$('#SelDistrictLoc').trigger('change');
                            var indNo=$('#SelSample').prop('selectedIndex')
                            $('#SelSample option:selected').remove();                            
                            $('#SelSample :nth-child('+(indNo+1)+')').prop('selected', true);
  ...
  $('#SelSample').change(function(e){
    var idtxt=$('#SelSample option:selected').val()
    var pos=idtxt.indexOf(',')
    var id=idtxt=idtxt.substring(1, pos);    
    getFPat19FromId(id)
  })

function listenImageLoadNew() {
    ...
    //setCurrentImageNew(image2)
    //new_file=File.createFromFileName("{% static '/Gmap/images/'%}{{image_name}}");
    //var new_file = new File(["foo"], "/Gmap/images/Test42.jpg")
...
(() => {
    ...
    document.onreadystatechange = () => {
        if (document.readyState === "complete") {
            ...
            // var filtered_estring=encodeURIComponent(encoded_string.replace(/\s/g, ''));
            // var blob = b64toBlob(filtered_estring, 'image/jpg');
            // var blobUrl = URL.createObjectURL(blob);
            // reader.readAsDataURL(blobUrl);
            //smthg=reader.readAsDataURL(encoded_string)
            //alert(smthg)
            //$('images').trigger('load');
           ...
           /* reader.addEventListener("load", () => {
             ...
             loadXHR(image_path).then(function(blob) {
    ...
    //added later
    function setCurrentImageNew(image)
    {
        ...
        con.drawImage(image, 0, 0, 640, 640);
        if (currentBbox !== null) {

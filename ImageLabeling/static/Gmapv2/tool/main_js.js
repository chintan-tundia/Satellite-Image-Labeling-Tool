(() => {
    ...
    const fontColor = "#ffffff" // Base font color
    const borderColor = "#001f3f" // Base bbox border color
    //const backgroundColor = "rgba(0, 116, 217, 0.2)" // Base bbox fill color
    const backgroundColor = "rgba(255, 255, 0, 0.2)" // Base bbox fill color rgb(255,255,0)
        ...
        //document.getElementById("images").addEventListener("change", (event) => {
           ...
           // if (files.length > 0) {
               ...
               // for (let i = 0; i < files.length; i++) {
                    const nameParts = image_name.split(".")                    
                ...
                for (let image in images) {
                    ...
                    //reader.addEventListener("load", () => {
                       ...
                       // imageObject.addEventListener("load", (event) => {
                            images[image].width = 640
                            images[image].height = 640
    ...
    const setCurrentImage = (image) => {
        ...
        //reader.addEventListener("load", () => {
            ...
            //imageObject.src = encoded_string

            document.getElementById("imageInformation")
                .innerHTML = `Image Dimensions: ${image.width} x ${image.height}`
            ...
            for (let imageName in bboxes) {
                ...
                /*for (let className in bboxes[imageName]) {
                    ...
                    for (let i = 0; i < bboxes[imageName][className].length; i++) {
                        ...
                        // sub_result["top_left_x"]=`${bbox.x}`;
                        // sub_result["top_left_y"]=`${bbox.y}`;
                        // sub_result["bottom_right_x"]=`${bbox.x + bbox.width}`;
                        // sub_result["bottom_right_y"]=`${bbox.y + bbox.height}`;  
                                               
                        result.csnm.push({
                            "top_left_x" : `${bbox.x}`,
                            "top_left_y" : `${bbox.y}`,
                            "bottom_right_x":`${bbox.x + bbox.width}`,
                ...
                for (let className in bboxes[imageName]) {
                    ...
                    for (let i = 0; i < bboxes[imageName][className].length; i++) {
                        ...
                        result+=`{"top_left_x":${bbox.x}, "top_left_y":${bbox.y},`;                        
                        result+=`"bottom_right_x":${bbox.x + bbox.width}, "bottom_right_y":${bbox.y + bbox.height}}`; 
                        if(i < (bboxes[imageName][className].length)-1){
                            result += ',';
                        }
                ...
                var bbox_json = JSON.stringify(jsonObject);                               

                $.ajax({
                    type:"POST",
                        url: 'ajax/save_annotations',
                        data: {
                        ...
                        success: function (data) {                
                            if (data.status==1) {
                              alert("Annotations are saved");                              
                              document.location.href="/Gmap/DisplayImages";                                         
                            }                            
                        },
